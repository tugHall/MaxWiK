...
```{r, include = TRUE}
cols = c(1:4, 9:16)
df = as.data.frame( MaxWiK::Data.2D$ABC$Matrix.Voronoi[ , cols ] )
names( df ) = cols
print( df )
...
```{r, include = TRUE}
MaxWiK::Data.2D$ABC$result.MaxWiK$Matrix_iKernel[1:5,]
...
posteriori.MaxWiK = MaxWiK::Data.2D$X[ which(MaxWiK::Data.2D$metasampling$result$iKernelABC$similarity > 0), ]
...
The red line is a true value, dotted is the best simulation, the red area is metasampling generated points, and the green area is the points taken from isolation kernel algorithm with similarity more than 0.
